...
float fill2(float x, float size, float edge) {
    return 1.0 - smoothstep(size - edge, size + edge, x);
}
float stroke(in float d, in float t) { return (1.0 - aastep(t, abs(d))); }
float stroke2(float x, float size, float w, float edge) {
...
void main() {
    ...
    // fix naming conflict resolution in macro overload logic
    float sdfCircle = fill2(
    ...
    if (VAR == 0) {
        ...
        sdf = fill2(sdf, 0.6, sdfCircle) * 1.2;
    } else if (VAR == 2) {
        sdf = sdCircle(st, vec2(0.5));
        sdf = strokeAA(sdf, 0.58, 0.02, sdfCircle) * 4.0;
    } else if (VAR == 3) {
        sdf = sdPoly(st - vec2(0.5, 0.45), 0.3, 3);
        sdf = fill2(sdf, 0.05, sdfCircle) * 1.4;
    }
